CREATE OR ALTER PROCEDURE [dbo].[Internal_GetDirectorPaymentRequestPaging](
@countSkip INT,
@pageSize INT, 
@userId NVARCHAR(MAX), 
...
BEGIN
    ...
    DECLARE @AccountantRejected INT = 2;
    DECLARE @ManagerApproved INT = 4;
    DECLARE @AccountantUpdateRequest INT = 7;
    SELECT
    ...
    WHERE
        pr.ReviewStatus NOT IN (@Pending, @ManagerReject, @AccountantRejected, @ManagerApproved, @AccountantUpdateRequest)
        AND
            (
                ISNULL(@userId, '') = ''
                OR EXISTS
    ...
    SELECT
        pr.Id, pr.Name, pr.CreateDate, ui.FullName AS FullName, ui2.FullName AS ReviewUserName, pr.ReviewStatus, pr.Type, prp.PaymentAmount, prp.ProposePaymentDate, prp.Note, @totalRecord AS TotalRecord
    FROM dbo.PaymentRequest pr
