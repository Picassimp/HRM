CREATE OR ALTER PROCEDURE [dbo].[Internal_Pa_GetMyAnnualPa](
@userId INT
)
AS
BEGIN
select pa.Id, pa.Name, pa.Year, pa.IsPublic, pa.IsCompleted, pd.UserId, pr.AssessUserId, ui.FullName, ph.Status, ph.OneToOneNote
from Pa pa
    ...
    where ((((pd.UserId = @userId and pr.AssessUserId Is Null) 
        ...
        and pa.Type = 0 and pa.IsPublic = 'true')
END
